...
class User:
    ...
    def validate_registration(data):
        ...
        if len(data['email']) < 3:
            ...
            valid_registration = False
        if len(data['dob']) == 0:
            flash("You must provide your Date of Birth.", "registration_error_dob")
            valid_registration = False
        elif datetime.datetime.today() - datetime.datetime.strptime(data['dob'], '%Y-%m-%d') < datetime.timedelta(days=6575): # approximation for 18 years old
            flash("You must be over 18 years old to register.", "registration_error_dob")
            valid_registration = False
